...
export default async function ExceptionsPage() {
  ...
  const exceptions = await loadExceptionsView(runtime.prisma, workspaceId);
  const now = new Date();
  ...
  return (
    <main className="app-main" id="main-content">
      ...
      <section className="card-grid" aria-label="Exception summary">
        <article className="card"><p className="metric-label">Awaiting decision</p><p className="metric-value">{awaiting}</p><Status tone={awaiting === 0 ? "positive" : "warning"}>{awaiting === 0 ? "No decision backlog" : "Decision required"}</Status></article>
      ...
      {exceptions.length === 0 ? <EmptyState title="No exception requests">Exception requests will appear after a finding is bound to a policy digest and submitted for a decision.</EmptyState> : (
