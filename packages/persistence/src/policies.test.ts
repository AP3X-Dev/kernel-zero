...
describe("policy lifecycle", () => {
  it("updates only a draft and records the current maker", async () => {
    ...
    await savePolicyDraft(client(tx), { actorUserId: AUTHOR, correlationId: CORRELATION, document: policy(), packId: "pack", workspaceId: WORKSPACE });
    expect(updateMany.mock.calls[0]?.[0].where).toMatchObject({ state: "draft", workspaceId: WORKSPACE });
  });

  it("enforces maker-checker in the approval write predicate and freezes canonical bytes", async () => {
    const revision = { id: "revision", revision: 1, authorId: AUTHOR, canonicalJson: JSON.stringify(policy()), state: "draft" };
    const updateMany = vi.fn<(input: { data: { canonicalJson: string }; where: { authorId: { not: string }; state: string } }) => Promise<{ count: number }>>().mockResolvedValue({ count: 1 });
    const tx = { auditRecord: { create: vi.fn() }, policyRevision: { findFirst: vi.fn().mockResolvedValue(revision), updateMany } };
    await expect(approvePolicyRevision(client(tx), { actorUserId: AUTHOR, correlationId: CORRELATION, revisionId: "revision", workspaceId: WORKSPACE })).rejects.toThrow("maker_checker");
    const approval = await approvePolicyRevision(client(tx), { actorUserId: CHECKER, correlationId: CORRELATION, revisionId: "revision", workspaceId: WORKSPACE });
    expect(approval.digest).toMatch(/^sha256:/u);
    expect(updateMany.mock.calls[0]?.[0].where).toMatchObject({ authorId: { not: CHECKER }, state: "draft" });
    expect(updateMany.mock.calls[0]?.[0].data.canonicalJson).toBe(canonicalJson(policy()));
  });

  it("activates one approved revision and accounts for a newly active pack atomically", async () => {
    const updateQuota = vi.fn<(input: { data: { used: { increment: number } } }) => Promise<{ count: number }>>().mockResolvedValue({ count: 1 });
    ...
    const tx = {
      $executeRaw: vi.fn().mockResolvedValue(1), auditRecord: { create: vi.fn() },
      ...
      quotaCounter: { updateMany: updateQuota },
    };
    await activatePolicyRevision(client(tx), { actorUserId: CHECKER, activePolicyLimit: 3, correlationId: CORRELATION, revisionId: "revision", workspaceId: WORKSPACE });
