...
3. Order of edits, smallest diff first:
   ...
   b. Action: `defineGovernedAction("<area>.<verb>", { audit: { actionCode, description, subjectType }, capability, idempotency, quota, tenantScope: "workspace", transactionTimeoutMs })`. All six metadata fields — audit, capability, idempotency, quota, tenantScope, transactionTimeoutMs — no nulls unless the ADR justifies it.
...
Never: bypass `requireCapability`, import @kernel-zero/persistence or @prisma/client from a route, page, or layout (rules ui-does-not-import-persistence, transport-does-not-import-repositories, raw-database-client-is-contained), add a `quota: null` without an ADR line, or add a network call to the control plane.
