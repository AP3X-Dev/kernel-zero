...
5. **Is this a new governed action?** If yes: `audit`, `idempotency`, `tenantScope`, `transactionTimeoutMs`, each stated explicitly (`transactionTimeoutMs` is required by `defineGovernedAction` even though the lint rule lists only three keys; ask for all four).
