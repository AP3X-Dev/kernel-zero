...
2. Copy the shape of `packages/profile-manifest` into `packages/profile-<name>`:
   ...
   - `check.ts`: pure function from inputs to `readonly EvidenceFinding[]`, sorted with `sortFindings`, identities from `findingIdentity`. No I/O inside the checker; the runner script reads files.
   - `index.ts`: `findingCompatibilityReason`, `diffRules`, and the frozen `Profile` object.
